...
void test1m(void) {
	...
	CREATE_PROCESS("server", client, NORMAL_PRIORITY, &Z502_REG3,
		&Z502_REG9);
	SuccessExpected(Z502_REG9, "CREATE_PROCESS");
	...
	for (Iteration = 1; Iteration <= 3; Iteration++) {
		if (Iteration == 1) {
			td->target_pid = Z502_REG3;
			strcpy(td->msg_sent, "11111111111");
		}
		if (Iteration == 2) {
			td->target_pid = Z502_REG4;
			strcpy(td->msg_sent, "22222222222");
		}
		if (Iteration == 3) {
			td->target_pid = Z502_REG5;
			strcpy(td->msg_sent, "33333333333");
		}
		td->send_length = 20;
		SEND_MESSAGE(td->target_pid, td->msg_sent, td->send_length, &Z502_REG9);
		SuccessExpected(Z502_REG9, "SEND_MESSAGE");
...
void client(void){
	//simulate a communication as test1j_echo
	CLIENT_DATA *c;                          // Use as ptr to data 
	
	c = (CLIENT_DATA *)calloc(1, sizeof(CLIENT_DATA));
	if (c == 0) {
	...
	while (1) {       // Loop forever.
		c->source_pid = -1;
		c->receive_length = LEGAL_MESSAGE_LENGTH;
		RECEIVE_MESSAGE(c->source_pid, c->msg_buffer, c->receive_length,
			&(c->actual_senders_length), &(c->actual_source_pid),
			&Z502_REG9);
		SuccessExpected(Z502_REG9, "RECEIVE_MESSAGE");

		printf("Receive from PID = %ld: length = %ld: msg = %s:\n",
			c->actual_source_pid, c->actual_senders_length,
			c->msg_buffer);

		c->target_pid = c->actual_source_pid;
		strcpy(c->msg_sent, c->msg_buffer);
		c->send_length = c->actual_senders_length;
		SEND_MESSAGE(c->target_pid, c->msg_sent, c->send_length, &Z502_REG9);
		SuccessExpected(Z502_REG9, "SEND_MESSAGE");
	}     // End of while
}
/**************************************************************************
